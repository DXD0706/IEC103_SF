...
void Service::setPointAddrMap(map<string,string> pointAddrmap)
{
	map<string,string>::iterator it = pointAddrmap.begin();
	for( ; it != pointAddrmap.end(); ++it)
	{
		Point103 point103;
		point103.pubAddr = it->second;
		m_pointAddrMap[it->first] = point103;
	}
}

bool Service::connectRedis(string ip, int port)
{
	m_pRedis = new IDB_Oper();
	if(!m_pRedis->Connect(1,ip.c_str(),port))
	{
		LOG_ERROR("1 NULL redis connect fail!");
		return false;
	}
	return true;
}

void Service::writePointsValue(std::vector<Tag>& m_tag_ver)
{
	for(int i = 0; i < m_tag_ver.size(); i++)
	{
		Tag tag = m_tag_ver[i];
		RtdbMessage rtdbMessage;
		RealPointValue* realPointValue = rtdbMessage.mutable_realpointvalue();
		realPointValue->set_channelname(Config::GetInstance()->GetChannelName());
		realPointValue->set_pointvalue(tag.pdata);
		realPointValue->set_pointaddr(tag.pName);
		//realPointValue->set_valuetype(type);
		realPointValue->set_channeltype(1);
		rtdbMessage.set_messagetype(TYPE_REALPOINT);

		char data[128] = {0};
		rtdbMessage.SerializeToArray(data,128);

		if(!m_pRedis->check_connection(1))
		{
			m_pRedis->DisConnect();
			if(m_pRedis->Connect(1,m_redisIp.c_str(),m_redisPort))
			{
				LOG_DEBUG("1 NULL main connect redis succeed");
			}
			else
			{
				LOG_DEBUG("1 NULL main connect redis fail");
			}
			continue;
		}

		m_pRedis->WriteValue(data,rtdbMessage.ByteSize(),m_posChannel,tag.pName);
	}

}

void Service::setChannel(vector<string> vecChannel)
{
	for(int i = 0 ; i < vecChannel.size(); ++i)
	{
		m_posChannel.push_back(vecChannel[i]);
	}
}

void Service::setChannel(string vecChannel)
{
	m_posChannel.push_back(vecChannel);
}
void Service::Start()
{
	string ip = Config::GetInstance()->GetIp();
	int port = Config::GetInstance()->GetUdpPort();
	m_net = new UdpNet( ip, port, 1888);

	WgjFunctionEx::GetInstance()->LogSend(LOGINFO::INFO,LOGINFO::NORMAL,string("103设备:").append(ip));
	int lastPos = ip.find_last_of('.') + 1;
	string addr = ip.substr(lastPos);
	//m_clientAddr = atoi(addr.c_str());
	m_clientAddr = 0x32;
	if(!m_net->Connect())
	{
...
bool Service::Write()
{
	...
	WgjFunctionEx::GetInstance()->LogSend(LOGINFO::INFO,LOGINFO::DATAFRAME,string("write: ").append(g_cmdName[m_nextCmd]) + " " + ToHexString(strCmd.data(), strCmd.size()).c_str());
	//TODO:测试时使用
...
bool Service::Read()
{
	...
	else
	{
		...
		WgjFunctionEx::GetInstance()->LogSend(LOGINFO::INFO,LOGINFO::DATAFRAME,string("read: ").append(g_cmdName[m_nextCmd]) + " " + ToHexString(m_datapacket, readSize).c_str());
...
void Service::ParseASDU44(const char* packet, int32_t length)
{
    ...
    for(int i = 0; i < scdNum; ++i)
    {
		...
		sprintf(addr, "%d_%d", fun, inf); //构造103地址
...
void Service::ParseASDU42(const char* packet, int32_t length)
{
    ...
    for(int i = 0; i < dpiNum; ++i)
    {
		...
		if(!strGlgAddr.empty())
		{
			...
			WgjFunctionEx::GetInstance()->LogSend(LOGINFO::ERROR,LOGINFO::NORMAL,string(" find redis:").append(tagTemp.pName));
			vecTag.push_back(tagTemp);
		}
		else
		{
			WgjFunctionEx::GetInstance()->LogSend(LOGINFO::ERROR,LOGINFO::NORMAL,string("not find addr:").append(addr));
		}
		++inf;
    }
    writePointsValue(vecTag);
...
void Service::ParseASDU1(const char* packet, int32_t length)
{
	...
	if(!strGlgAddr.empty())
	{
		...
		WgjFunctionEx::GetInstance()->LogSend(LOGINFO::ERROR,LOGINFO::NORMAL,string(" find redis:").append(tagTemp.pName));
		vecTag.push_back(tagTemp);
		writePointsValue(vecTag);; //更新点到内存数据库
	}
	else
	{
		WgjFunctionEx::GetInstance()->LogSend(LOGINFO::ERROR,LOGINFO::NORMAL,string("not find addr:").append(addr));
...
void Service::ParseASDU2(const char* packet, int32_t length)
{
	...
	if(!strGlgAddr.empty())
	{
		...
		sprintf(tagTemp.pdata, "%d", bValue);
		vecTag.push_back(tagTemp);
		std::cout<<"save redis database tagName ="<<tagTemp.pName<<" tagValue ="<<tagTemp.pdata<<std::endl;
		writePointsValue(vecTag); //更新点到内存数据库
	}
	else
	{
...
void Service::ParseASDU10AllValue(const char* packet, int32_t length)
{
	...
	for(int i = 0; i < ngdNum; ++i)
	{
		...
		char addr[100] = {0};
		memset(addr, 0, sizeof(addr));
		sprintf(addr, "%d_%d",dataUnit.groupNo, dataUnit.entryNo); //构造103地址
